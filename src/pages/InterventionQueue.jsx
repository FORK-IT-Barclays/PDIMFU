...
import { Phone, RefreshCcw, Search, ShieldAlert, Filter, Check, Send, ChevronDown, X, Download, Users } from 'lucide-react';
import { mockCustomers, batchProcessor } from '../data/mockData';

const InterventionQueue = () => {
    // Filters
    const [tierFilter, setTierFilter] = useState('all'); // 'all', 'Tier 3', 'Tier 2', 'Tier 1'
    const [diagnosisFilter, setDiagnosisFilter] = useState('all');
    const [riskRange, setRiskRange] = useState([0, 100]);
    const [velocityMin, setVelocityMin] = useState('');
    const [sortBy, setSortBy] = useState('urgency'); // 'urgency', 'risk', 'velocity', 'name'
    ...
    const allDiagnoses = useMemo(() => {
        return [...new Set(mockCustomers.map(c => c.diagnosis))].sort();
    }, []);
    ...
    const filteredCustomers = useMemo(() => {
        ...
        return list;
    }, [tierFilter, diagnosisFilter, riskRange, velocityMin, sortBy, sortDir, searchQuery]);
    ...
    const selectAllVisible = () => {
        const allIds = new Set(shortlisted);
        filteredCustomers.forEach(c => allIds.add(c.id));
        setShortlisted(allIds);
    };
    ...
    const shortlistedCustomers = mockCustomers.filter(c => shortlisted.has(c.id));

    const tierCounts = useMemo(() => ({
        all: mockCustomers.length,
        'Tier 3': mockCustomers.filter(c => c.tier === 'Tier 3').length,
        'Tier 2': mockCustomers.filter(c => c.tier === 'Tier 2').length,
        'Tier 1': mockCustomers.filter(c => c.tier === 'Tier 1').length,
    }), []);
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="page-container">
                    ...
                    <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
                        <div className="glass" style={{ padding: '0.4rem 0.8rem', borderRadius: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem', width: '220px' }}>
                            ...
                            <input
                                ...
                                className="mono"
                                style={{ background: 'none', border: 'none', outline: 'none', color: 'white', fontSize: '0.7rem', width: '100%' }}
                            />
                        </div>
                        <button
                    ...
                    <button
                        ...
                        style={{
                            ...
                            background: tierFilter === t
                                ? (t === 'Tier 3' ? 'var(--tier-3-soft)' : t === 'Tier 2' ? 'var(--tier-2-soft)' : t === 'Tier 1' ? 'var(--tier-1-soft)' : 'var(--primary)')
                                : 'rgba(255,255,255,0.02)',
                            border: tierFilter === t
                                ? (t === 'Tier 3' ? '1px solid var(--tier-3)' : t === 'Tier 2' ? '1px solid var(--tier-2)' : t === 'Tier 1' ? '1px solid var(--tier-1)' : '1px solid var(--primary)')
                                : '1px solid var(--border)',
                            color: tierFilter === t
                                ? (t === 'Tier 3' ? 'var(--tier-3)' : t === 'Tier 2' ? 'var(--tier-2)' : t === 'Tier 1' ? 'var(--tier-1)' : 'black')
                                : 'var(--text-dim)',
                        }}
                    >
                        ...
                        <div className="technical-card" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1.25rem', padding: '1.25rem' }}>
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    value={riskRange[0]}
                                    onChange={e => setRiskRange([+e.target.value, riskRange[1]])}
                                    className="mono"
                            ...
                            <div>
                                ...
                                <input
                                    type="number" min={0} max={100}
                                    value={riskRange[1]}
                                    onChange={e => setRiskRange([riskRange[0], +e.target.value])}
                                    className="mono"
                                    style={{ width: '100%', background: 'var(--bg-surface)', border: '1px solid var(--border)', color: 'white', padding: '0.4rem', borderRadius: '4px', fontSize: '0.7rem' }}
                                />
                            </div>
            ...
            <div className="content-grid" style={{ gridTemplateColumns: '1fr 340px', gap: '2rem' }}>
                <main className="main-column">
                        ...
                        <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                            ...
                            {[
                                ...
                                <button
                                    key={s.key}
                                    onClick={() => {
                                        if (sortBy === s.key) setSortDir(d => d === 'desc' ? 'asc' : 'desc');
                                        else { setSortBy(s.key); setSortDir('desc'); }
                                    }}
                                    className="mono"
                                    style={{
                                        padding: '0.2rem 0.5rem', borderRadius: '3px', cursor: 'pointer', fontSize: '0.55rem', fontWeight: 700,
                                        background: sortBy === s.key ? 'rgba(56, 189, 248, 0.1)' : 'transparent',
                                        border: sortBy === s.key ? '1px solid rgba(56, 189, 248, 0.3)' : '1px solid transparent',
                                        color: sortBy === s.key ? 'var(--primary)' : 'var(--text-dim)',
                                    }}
                                >
                        ...
                        <table className="institutional-table">
                            ...
                            <tbody>
                                {filteredCustomers.map(c => {
                                    ...
                                    return (
                                        <tr key={c.id} style={{ cursor: 'pointer', background: isSelected ? 'rgba(56,189,248,0.04)' : 'transparent' }}
                                            ...
                                            <td>
                                                <div className="mono" style={{
                                                    ...
                                                    color: parseFloat(urgency) > 0.5 ? 'var(--tier-3)' : parseFloat(urgency) > 0.2 ? 'var(--tier-2)' : 'var(--tier-1)'
                                                }}>
                                                    {urgency}U
                                                </div>
                                            </td>
                                            <td>
                                                <div className="mono" style={{ fontSize: '0.65rem', color: 'var(--text-muted)', fontWeight: 700 }}>{c.diagnosis}</div>
                                            </td>
                                            <td>
                                                <span className={`tier-tag ${c.tier.toLowerCase().replace(' ', '')}`}>{c.tier}</span>
                                            </td>
                    ...
                    <div className="mono text-dim" style={{ fontSize: '0.6rem', marginTop: '0.5rem', textAlign: 'right' }}>
                        Showing {filteredCustomers.length} of {mockCustomers.length} monitored loan accounts
                    </div>
                ...
                <aside className="side-column">
                    {/* Shortlist / Batch Panel */}
                    <div className="technical-card" style={{
                        background: shortlisted.size > 0 ? 'rgba(56, 189, 248, 0.03)' : 'rgba(0,0,0,0.1)',
                        borderColor: shortlisted.size > 0 ? 'rgba(56, 189, 248, 0.2)' : 'var(--border)',
                        transition: 'all 0.3s'
                    }}>
                        <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center', marginBottom: '1rem' }}>
                            ...
                            {shortlisted.size > 0 && (
                                <span className="mono" style={{ fontSize: '0.6rem', fontWeight: 800, color: 'var(--primary)', marginLeft: 'auto' }}>
                        ...
                        {shortlisted.size === 0 ? (
                            <p style={{ fontSize: '0.7rem', color: 'var(--text-dim)', lineHeight: 1.5, margin: 0 }}>
                                Select loan accounts from the table to shortlist them for batch intervention. Use filters to narrow down by risk score, tier, or diagnosis.
                            </p>
                        ) : (
                            <>
                                <div style={{ maxHeight: '150px', overflowY: 'auto', marginBottom: '1rem' }}>
                                    {shortlistedCustomers.map(c => (
                                        <div key={c.id} style={{
                                            ...
                                            padding: '0.4rem 0', borderBottom: '1px solid var(--border)', fontSize: '0.7rem'
                                        }}>
                                            <div>
                                                <span style={{ fontWeight: 700 }}>{c.name}</span>
                                                <span className="mono text-dim" style={{ fontSize: '0.55rem', marginLeft: '0.5rem' }}>{(c.risk * 100).toFixed(0)}%</span>
                                            </div>
                                            <button onClick={(e) => { e.stopPropagation(); toggleShortlist(c.id); }} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '0.2rem' }}>
                                ...
                                <div style={{ marginBottom: '1rem' }}>
                                    ...
                                    >
                                        ...
                                        <option value="EMI_HOLIDAY">EMI_HOLIDAY — 30-Day Pause</option>
                                        <option value="RESTRUCTURE">RESTRUCTURE — Loan Restructuring</option>
                                    </select>
                                </div>

                                <button
                                    onClick={handleBatchSend}
                                    disabled={batchSending || batchSent}
                                    className="mono"
                                    style={{
                                        width: '100%', padding: '0.75rem', borderRadius: '0.5rem', cursor: 'pointer',
                                        background: batchSent ? 'var(--tier-1)' : batchSending ? 'var(--bg-surface)' : 'var(--primary)',
                                        border: 'none', color: batchSent ? 'white' : 'black',
                                        fontSize: '0.7rem', fontWeight: 900, letterSpacing: '0.05em',
                                        display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem',
                                        transition: 'all 0.3s',
                                    }}
                                >
                                    {batchSent ? <><Check size={14} /> BATCH DISPATCHED</> :
                                        batchSending ? <><RefreshCcw size={14} className="spin-icon" /> PROCESSING...</> :
                                            <><Send size={14} /> DISPATCH BATCH ({shortlisted.size} ACCOUNTS)</>}
                                </button>
                            </>
                        )}
                    </div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                            <div>
                                ...
                                <p style={{ fontSize: '0.7rem', color: 'var(--text-dim)', margin: 0, lineHeight: 1.4 }}>
                                    Urgency &gt; 1.0U requires mandatory Relationship Manager contact within 4 hours.
                                </p>
                            </div>
                            <div>
                                <div className="stat-label" style={{ fontSize: '0.55rem', color: 'white', marginBottom: '0.25rem' }}>Action</div>
                                <p style={{ fontSize: '0.7rem', color: 'var(--text-dim)', margin: 0, lineHeight: 1.4 }}>
                                    Initiating "EXECUTE_CALL" auto-generates a dossier and opens a secure bridge.
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="technical-card">
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                            {[
                                ...
                                { label: 'T3 Critical', val: mockCustomers.filter(c => c.tier === 'Tier 3').length, color: 'var(--tier-3)' },
                                { label: 'T2 Warning', val: mockCustomers.filter(c => c.tier === 'Tier 2').length, color: 'var(--tier-2)' },
                                { label: 'T1 Watch', val: mockCustomers.filter(c => c.tier === 'Tier 1').length, color: 'var(--tier-1)' },
                            ].map((s, i) => (
                    ...
                    <div className="technical-card glass" style={{ borderColor: 'var(--text-dim)', opacity: 0.6 }}>
                        <p style={{ fontSize: '0.65rem', color: 'var(--text-dim)', lineHeight: 1.5, margin: 0 }}>
                            Data from latest batch <span className="mono" style={{ color: 'white' }}>{batchProcessor.lastBatchId}</span>.<br />
                            Last sector sync: 12m ago. Portfolio drift: <span className="text-red">+0.002</span>.
                        </p>
                    </div>
                </aside>
            </div>
            ...
            <style dangerouslySetInnerHTML={{
                __html: `
              .tier-tag { font-size: 0.55rem; padding: 0.15rem 0.5rem; border-radius: 4px; font-weight: 900; display: inline-block; text-transform: uppercase; border: 1px solid transparent; }
              .tier-tag.tier3 { background: var(--tier-3-soft); color: var(--tier-3); border-color: rgba(244, 63, 94, 0.2); }
              .tier-tag.tier2 { background: var(--tier-2-soft); color: var(--tier-2); border-color: rgba(251, 191, 36, 0.2); }
              .tier-tag.tier1 { background: var(--tier-1-soft); color: var(--tier-1); border-color: rgba(16, 185, 129, 0.2); }
