...
import {
    ...
    TrendingDown, FileText, ChevronRight, Play, Info
} from 'lucide-react';
...
const CustomerSearch = () => {
    ...
    const getChartData = (customer, range, simulationId = null) => {
        const data = customer.history.slice(-range).map((val, i) => ({
            day: i,
            risk: parseFloat(val.toFixed(2)),
            projected: null,
            date: new Date(Date.now() - (range - i) * 24 * 60 * 60 * 1000).toLocaleDateString('en-IN', {
        ...
        if (simulationId) {
            ...
            // Bridge point: last historical point also becomes the first projected point
            ...
            for (let i = 1; i <= points; i++) {
                ...
                const projectedVal = Math.max(15, lastRisk - reduction + (Math.random() * 2));
    ...
    const currentChartData = useMemo(() => {
        if (!selected) return [];
        return getChartData(selected, timeRange === 'max' ? selected.history.length : timeRange, simulating);
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [selected, timeRange, simulating]);

    const handleSimulate = (id) => {
        setSimulating(id === simulating ? null : id);
    };
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="page-container">
            <header className="page-header" style={{ marginBottom: '2.5rem' }}>
                <div>
                    ...
                    <p className="mono text-dim" style={{ fontSize: '0.75rem', marginTop: '0.2rem' }}>MONITORED_LOAN_ACCOUNTS // BATCH_PROCESSED_DATA</p>
                </div>
                {!selected && (
                    <div className="search-box glass" style={{ padding: '0.5rem 1rem', borderRadius: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.75rem', width: '320px' }}>
                        ...
                        <input
                            type="text"
                            placeholder="Find loan account (ID or name)..."
                            value={query}
                            onChange={(e) => { setQuery(e.target.value); }}
                            className="mono"
                            style={{ background: 'none', border: 'none', outline: 'none', color: 'white', fontSize: '0.8rem', width: '100%' }}
                        />
                    </div>
                )}
            </header>
                        ...
                        <div className="content-grid" style={{ gridTemplateColumns: '1fr 320px', overflow: 'visible' }}>
                                ...
                                <div className="technical-card">
                                    ...
                                    <div style={{ background: 'rgba(0,0,0,0.2)', border: '1px solid var(--border)', borderRadius: '1rem', padding: '1.5rem', marginBottom: '2rem' }}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
                                            <div style={{ display: 'flex', gap: '0.25rem' }}>
                                                {[14, 30, 'max'].map(r => (
                                                    <button key={r} onClick={() => setTimeRange(r)} className={`mono ${timeRange === r ? 'active' : ''}`} style={{ padding: '0.3rem 0.75rem', borderRadius: '4px', border: 'none', background: timeRange === r ? 'var(--primary)' : 'rgba(255,255,255,0.03)', color: timeRange === r ? 'black' : 'var(--text-dim)', fontSize: '0.65rem', fontWeight: 800, cursor: 'pointer' }}>
                                                        {r === 'max' ? 'MAX' : `${r}D`}
                                                    </button>
                                                ))}
                                            </div>
                                        ...
                                        <div style={{ height: 280 }}>
                                            <ResponsiveContainer width="100%" height="100%">
                                                <AreaChart data={currentChartData}>
                                                    ...
                                                    <Tooltip
                                                        contentStyle={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '0.5rem', fontSize: '10px' }}
                                                        formatter={(value, name) => {
                                                            if (value === null) return [null, null];
                                                            const label = name === 'projected' ? 'Projected Risk' : 'Risk Score';
                                                            return [`${value}%`, label];
                                                        }}
                                                    />
                                                    {simulating && (
                                                        <ReferenceLine
                                                            x={currentChartData.find(d => d.projected !== null)?.date}
                                                            stroke="rgba(255,255,255,0.1)"
                                                            strokeDasharray="3 3"
                                                            label={{ value: 'SIM START', fill: 'var(--text-dim)', fontSize: 9, position: 'top' }}
                                                        />
                                                    )}
                                                    <Area type="monotone" dataKey="risk" stroke="var(--primary)" fill="url(#colorRisk)" strokeWidth={2} connectNulls={false} />
                                                    {simulating && (
                                                        <Area type="monotone" dataKey="projected" stroke="var(--tier-1)" fill="url(#colorSim)" strokeWidth={2} strokeDasharray="6 3" connectNulls={false} />
                                                    )}
                                                </AreaChart>
                                            </ResponsiveContainer>
                                        </div>
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.25rem' }}>
                                        {[
                                            ...
                                            { label: 'Interventions', val: selected.interventions.length, sub: 'Historical total' },
                                        ].map((m, i) => (
                            ...
                            <aside className="side-column">
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                                        {interventionStrategies.map(s => (
                                            <button
                                                ...
                                                style={{
                                                    padding: '1rem', background: simulating === s.id ? 'var(--tier-1-soft)' : 'rgba(255,255,255,0.01)',
                                                    border: simulating === s.id ? '1px solid var(--tier-1)' : '1px solid var(--border)',
                                                    borderRadius: '0.75rem', color: 'white', textAlign: 'left', cursor: 'pointer', transition: 'all 0.2s'
                                                }}
                                            >
                                ...
                                <button style={{
                                    ...
                                    border: 'none', color: 'black', borderRadius: '0.75rem',
                                    display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.75rem',
                                    fontSize: '0.85rem', fontWeight: 900, cursor: 'pointer', marginTop: 'auto'
                                }}>
            ...
            <style dangerouslySetInnerHTML={{
              ...
              .back-link { border: none; background: none; color: var(--text-dim); font-weight: 800; font-size: 0.7rem; cursor: pointer; letter-spacing: 0.15em; transition: color 0.2s; }
